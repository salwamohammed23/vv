    ...
    if file is not None:
        ...
        if st.button('Generate EDA'):
            ...
            if model_type == 'Regression':
                selected_models = st.multiselectbox("Select models", ['gbr', 'lightgbm', 'xgboost', 'lar', 'llar', 'et', 'en', 'lasso', 'knn', 
                                                                  'ada', 'omp', 'par', 'huber', 'dt', 'lr', 'br', 'rf', 'ridge', 'dummy'])
                models.update({model: True for model in selected_models})

            if model_type == 'Classification':
                selected_models = st.multiselectbox("Select models", ['gbc', 'lightgbm', 'ada', 'xgboost', 'lda', 'et', 'gbc', 'ada', 'knn', 'lr', 'rf', 'ridge', 'dummy'])
                models.update({model: True for model in selected_models})
